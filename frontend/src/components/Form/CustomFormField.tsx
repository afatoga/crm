...
export const CustomFormField = ({ controllerProps, fieldData, errors }) => {
  ...
  const partyRelationshipTypeList =
    fieldData.name === "partyRelationshipTypeId" ? fieldData.apiRequest() : [];
  ...
  const getOptionLabel = (option: number | { id: string; name: string }) => {
    if (
      !option ||
      (!fieldData.apiRequest.data &&
        fieldData.name !== "partyRelationshipTypeId" &&
        fieldData.name !== "partyRelationshipId")
    )
      return "";
    ...
    if (typeof option === "number") {
      if (fieldData.name === "statusId") {
        ...
        if (fieldData.apiRequest.length) {
          ...
          if (found)
            return parseInt(fieldData.currentPartyId) === found.firstPartyId
        ...
        if (fieldData.apiRequest.data?.contactTypeList?.length) {
          const list = fieldData.apiRequest.data.contactTypeList;
          const found = list.find((item: any) => option === parseInt(item.id));
          if (found) return found.name;
        }
      }
  ...
  if (fieldData.type === "text") {
    return (
      <TextField
        ...
        placeholder={fieldData.name === "birthday" ? "yyyy-mm-dd" : undefined}
        helperText={
